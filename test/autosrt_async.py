...
async def main():
    ...
    for media_filepath in media_filepaths:
        ...
        get_transcription_async_partial = partial(recognizer.get_transcription_async, recognizer=recognizer)
        ...
        if regions:
            try:
                ...
                pbar = ProgressBar(widgets=widgets, maxval=len(regions)).start()
                transcripts = []
                for i, transcript in enumerate(pool.imap(get_transcription_async_partial, extracted_regions)):

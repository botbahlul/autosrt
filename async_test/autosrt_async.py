...
class Language:
    ...
    def __init__(self):
        ...
        self.list_codes = list(self.dict.keys())
        self.list_names = list(self.dict.values())
        self.code_of_name = {name: code for code, name in self.dict.items()}
        self.name_of_code = dict(self.dict)

    async def get_name(self, get_code):
        if not get_code:
            return ""
        return self.dict.get(get_code, self.dict.get(get_code.lower(), ""))
...
class WavConverter:
    ...
    async def __call__(self, media_filepath):
        ...
        fd, wav_filepath = tempfile.mkstemp(suffix=".wav")
...
class SpeechRegionFinder:
    ...
    async def __call__(self, wav_filepath):
        reader = None
        try:
            reader = wave.open(wav_filepath, "rb")
            sample_width = (reader.getsampwidth())
            rate = (reader.getframerate())
            n_channels = (reader.getnchannels())
            total_frames = (reader.getnframes())
            total_duration = (float(total_frames)/float(rate))
            chunk_duration = (float(self.frame_width)/float(rate))
            energies = []
            ...
            threshold = (self.percentile(energies, 0.20))
            regions = []
            elapsed_time = 0.0
            region_start = None
            ...
            for energy in energies:
                ...
                if region_start is not None:
                    region_duration = (elapsed_time - region_start)
                    max_exceeded = (region_duration>= self.max_region_size)
                    if (max_exceeded or is_silence):
                        if (region_duration >= self.min_region_size):
            ...
            if region_start is not None:
                ...
                if (end_time - region_start >= self.min_region_size):
...
class FLACConverter:
    ...
    async def __call__(self, region):
        ...
        fd, flac_filepath = tempfile.mkstemp(suffix=".flac")
        os.close(fd)
        ...
        try:
            process = (await asyncio.create_subprocess_exec(*command,stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE))
            _, stderr = (await process.communicate())
            if process.returncode != 0:
                error_text = (stderr.decode("utf-8", errors="replace") if stderr else "ffmpeg failed")
                raise RuntimeError(error_text)

            with open(flac_filepath, "rb") as f:
                content = f.read()
...
class SpeechRecognizer:
    def __init__(self, language="en", rate=48000, retries=3, api_key=GOOGLE_SPEECH_API_KEY, timeout=30, error_messages_callback=None):
    ...
    async def __call__(self, data):
        ...
        params = {"client": "chromium", "lang": self.language, "key": self.api_key}
        headers = {"Content-Type": "audio/x-flac; rate=%d" % self.rate}
        full_url = (url + "?" + "&".join("{}={}".format(k, v) for k, v in params.items()))

        for attempt in range(self.retries):
            try:
                response = await asyncio.to_thread(self._request, full_url, data, headers)
...
def parse_google_translation(data, endpoint_type):
...
class SentenceTranslator:
    def __init__(self, src, dst, endpoint_config, patience=-1, timeout=30, error_messages_callback=None):
        self.src = src
        self.dst = dst
    ...
    async def __call__(self, sentence):
        ...
        try:
            ...
            while (translated_sentence.endswith("\n") and patience):
    ...
    async def _translate(self, sentence):
        return await self.GoogleTranslate(sentence, self.src, self.dst, self.timeout)
    ...
    async def GoogleTranslate(self,  text, src, dst, timeout=30):
        ...
        try:
            ...
            return parse_google_translation(data, endpoint_type)
...
async def translate_subtitles(subtitles, translator, concurrency=10):
    semaphore = asyncio.Semaphore(max(1, concurrency))

    async def translate_one(index, text):
        async with semaphore:
            result = await translator(text)
            return index, result

    tasks = [asyncio.create_task(translate_one(index, text)) for index, text in enumerate(subtitles)]
    results = [None for _ in subtitles]
    completed = 0
...
class SubtitleFormatter:
    ...
    def srt_formatter(self, subtitles, padding_before=0, padding_after=0):
        ...
        for index, ((start, end), text) in enumerate(subtitles, start=1):
            ...
            start_ms = max(0, self._seconds_to_milliseconds(start - padding_before))
            end_ms = (self._seconds_to_milliseconds(end + padding_after))
            item.start = (pysrt.SubRipTime(milliseconds=start_ms))
            item.end = (pysrt.SubRipTime(milliseconds=end_ms))
    ...
    def json_formatter(self, subtitles):
        ...
        return json.dumps(subtitle_dicts, ensure_ascii=False)

    def raw_formatter(self, subtitles):
        return " ".join(six.text_type(text) for (_rng, text) in subtitles)
...
class SubtitleWriter:
    def __init__(self, regions, transcripts, format, error_messages_callback=None):
        ...
        self.format = format
        self.timed_subtitles = [(region, transcript)
            for region, transcript
            in zip(regions, transcripts)
    ...
    async def write(self, declared_subtitle_filepath):
        formatter = SubtitleFormatter(self.format, self.error_messages_callback)
        formatted_subtitles = await formatter(self.timed_subtitles)
        saved_subtitle_filepath = (declared_subtitle_filepath)
        base, ext = os.path.splitext(saved_subtitle_filepath)

        if not ext:
            saved_subtitle_filepath = ("{}.{}".format(base,self.format))
...
async def process_media_file(media_filepath, args, endpoint_config):
    ...
    try:
        wav_filepath, sample_rate = (await wav_converter(media_filepath))
    finally:
        if pbar is not None:
            pbar.finish()
        pbar = None
    ...
    async def extract_one(index, region):
        async with semaphore:
            result = await flac_converter(region)
            return index, result

    flac_tasks = [asyncio.create_task(extract_one(index, region)) for index, region in enumerate(regions)]
    extracted_regions = [None for _ in regions]
    completed = 0

    for task in asyncio.as_completed(flac_tasks):
    ...
    recognizer = SpeechRecognizer(language=args.src_language, rate=sample_rate, api_key=GOOGLE_SPEECH_API_KEY, retries=3, timeout=30, error_messages_callback=show_error_messages)
    widgets = ["Performing speech recognition           : ", Percentage(), " ", Bar(), " ", ETA()]
    pbar = ProgressBar(widgets=widgets, maxval=len(regions)).start()

    async def recognize_one(index, data):
        async with semaphore:
            if not data:
                result = None
            else:
                result = await recognizer(data)
            return index, result

    recognition_tasks = [asyncio.create_task(recognize_one(index, data)) for index, data in enumerate(extracted_regions)]
    transcripts = [None for _ in regions]
    completed = 0

    for task in asyncio.as_completed(recognition_tasks):
        index, transcript = (await task)
    ...
    subtitle_format = (args.format.lower())

    if args.output:
        subtitle_file_base, subtitle_ext = (os.path.splitext(args.output))

        if subtitle_ext:
            subtitle_filepath = (args.output)
        else:
            subtitle_filepath = ("{}.{}".format(args.output,subtitle_format))

    else:
        base, _ = os.path.splitext(media_filepath)
        subtitle_filepath = ("{}.{}".format(base, subtitle_format))
    ...
    if (args.dst_language and endpoint_config and not is_same_language(args.src_language, args.dst_language)):
        ...
        created_regions = [entry[0] for entry in timed_subtitles]
        created_subtitles = [entry[1] for entry in timed_subtitles]
        prompt = "Translating from %8s to %8s   : " %(args.src_language, args.dst_language)
        pbar = ProgressBar(widgets=[prompt, Percentage(), " ", Bar(), " ", ETA()],maxval=len(created_subtitles)).start()
        ...
        translated_subtitles = (await translate_subtitles(created_subtitles, translator, args.concurrency))
        pbar.finish()
        pbar = None

        subtitle_file_base, _ = (os.path.splitext(subtitle_filepath))
        translated_subtitle_filepath = ("{}.translated.{}".format(subtitle_file_base, subtitle_format))
...
async def main():
    ...
    parser.add_argument("source_path", help=("File path of the video or audio files to generate subtitles"), nargs="*")
    parser.add_argument("-S", "--src-language", help=("Language code of the audio language spoken in source"), default="en")
    parser.add_argument("-D", "--dst-language", help=( "Desired translation language code"), default=None)
    parser.add_argument("-ll", "--list-languages", help=("List all supported languages"), action="store_true")
    parser.add_argument("-o", "--output", help=("Output file path for subtitles"))
    ...
    # SOURCE LANGUAGE
    # ============================================================
    if (args.src_language not in language.name_of_code):
    ...
    if args.dst_language:
        if (args.dst_language not in language.name_of_code):
    ...
    for arg in args.source_path:
        ...
        for filepath in matches:
            ...
            if (is_video_file(filepath) or is_audio_file(filepath)):
                media_filepaths.append(filepath)
            else:
                print("{} is not a valid video or audio file".format(filepath))
    ...
    if (args.dst_language and not is_same_language(args.src_language, args.dst_language)):
    ...
    try:
        for media_filepath in media_filepaths:
            ...
            try:
                await process_media_file(media_filepath, args, endpoint_config)

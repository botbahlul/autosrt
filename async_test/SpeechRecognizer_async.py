...
class FLACConverter:
    ...
    async def convert(regions, converter):
        return await converter(regions)

    @staticmethod
    def convert_async(regions, converter):
        return asyncio.run(FLACConverter.convert(regions, converter))
...
async def main():
    ...
    converter_partial = partial(converter.convert_async, converter=converter)

    widgets = ["Converting speech regions to FLAC files : ", Percentage(), ' ', Bar(), ' ', ETA()]
    pbar = ProgressBar(widgets=widgets, maxval=len(regions)).start()

    extracted_regions = []
    for i, extracted_region in enumerate(pool.imap(converter_partial, regions)):
    ...
    transcripts = []
    #for i, transcript in enumerate(pool.imap(get_transcription_async_partial, extracted_regions)): # WORKING
